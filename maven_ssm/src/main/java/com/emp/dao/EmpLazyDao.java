...
public interface EmpLazyDao {
	...
	@Results(id="empMapper",value={
			...
			@Result(column="deptno",property="dept",one=@One(
			select="com.emp.dao.DeptDao.queryById",fetchType=FetchType.LAZY)),
			//fetchType=FetchType.LAZY只有调用的时候才会查询 many=@Many一对多集合
			//如果去掉fetchType=FetchType.LAZY不调用也会自动查询
			@Result(column="mgrno",property="mgr",one=@One(
			select="com.emp.dao.EmpLazyDao.queryById",fetchType=FetchType.LAZY))
	})
	...
	//添加员工
	@Insert("insert into t_emp(empno,ename,esex,eage,esalary,deptno,mgrno) values("
			+ "#{empno},#{ename},#{esex},#{eage},#{esalary},"
	...
	@ResultMap("empMapper")
	List<Emp> queryByDept(@Param("deptno")String deptno);
