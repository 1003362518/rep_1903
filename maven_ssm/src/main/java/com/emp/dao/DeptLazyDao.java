...
public interface DeptLazyDao {
		...
		@Results(id="deptMapper",value={
				...
				@Result(column="deptno",property="emps",many=@Many(select=
				"com.emp.dao.EmpDao.queryByDept",fetchType=FetchType.LAZY)),
		})
		List<Dept> queryAll();
		//依据部门编号查询部门
		...
		@ResultMap("deptMapper")
		List<Dept> queryByDeptno(@Param("deptno")String deptno);
